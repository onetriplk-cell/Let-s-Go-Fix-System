...
export default function RequestPage() {
  ...
  return (
    <div className="sm:pl-60">
      ...
      <div className="mx-auto flex max-w-md gap-1 px-4 pt-2 sm:max-w-2xl sm:px-8">
        {steps.map((s, i) => (
          <div
            key={s}
            className={cn('h-1 flex-1 rounded-full', i <= stepIndex ? 'bg-brand-600 dark:bg-brand-500' : 'bg-slate-200 dark:bg-slate-800')}
          />
        ))}
      </div>
      ...
      <div className="mx-auto flex max-w-md flex-col gap-4 p-4 sm:max-w-2xl sm:p-8">
        {step === 'vehicle' && (
          <>
            <h2 className="text-base font-semibold text-slate-900 dark:text-slate-100">Which vehicle needs help?</h2>
            {vehicles?.length === 0 && (
              <Card>
                <CardContent className="flex flex-col items-center gap-3 py-8 text-center">
                  <Car className="h-8 w-8 text-slate-300 dark:text-slate-700" />
                  <p className="text-sm text-slate-500 dark:text-slate-400">You need to add a vehicle first.</p>
            ...
            {vehicles?.map((v) => (
              <Card
                ...
                className={cn(
                  ...
                  vehicleId === v.id && 'border-brand-500 ring-2 ring-brand-100 dark:border-brand-400 dark:ring-brand-500/20',
                )}
        ...
        {step === 'category' && (
          <>
            <h2 className="text-base font-semibold text-slate-900 dark:text-slate-100">What do you need?</h2>
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
              {categories?.map((c) => (
                <Card
                  ...
                  className={cn(
                    'cursor-pointer transition-colors',
                    categoryId === c.id && 'border-brand-500 ring-2 ring-brand-100 dark:border-brand-400 dark:ring-brand-500/20',
                  )}
        ...
        {step === 'location' && (
          <>
            <h2 className="text-base font-semibold text-slate-900 dark:text-slate-100">Where are you?</h2>
            <p className="text-sm text-slate-500 dark:text-slate-400">Tap on the map to set your exact pickup location.</p>
            ...
            {coords && (
              <div className="flex items-start gap-2 rounded-xl bg-slate-100 p-3 text-sm text-slate-600 dark:bg-slate-900 dark:text-slate-300">
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Notes for the provider (optional)</label>
              <textarea
                rows={3}
                value={description}
                onChange={(e) => setDescription(e.target.value)}
                placeholder="e.g. Front-left tyre is flat, car is on the shoulder"
                className="flex w-full rounded-xl border border-slate-300 bg-white px-3.5 py-2 text-base shadow-sm placeholder:text-slate-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:placeholder:text-slate-500"
              />
            </div>
          </>
        )}
        ...
        {step === 'confirm' && (
          <>
            <h2 className="text-base font-semibold text-slate-900 dark:text-slate-100">Confirm your request</h2>
            <Card>
              <CardContent className="flex flex-col gap-3 pt-4 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-slate-900 dark:text-slate-100">{selectedCategory?.name}</span>
                </div>
                <div className="flex justify-between">
