...
>(({ className, children, ...props }, ref) => (
  <DialogPrimitive.Portal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        'fixed inset-x-4 bottom-4 z-50 rounded-2xl border border-slate-200 bg-white p-5 shadow-xl sm:inset-x-auto sm:bottom-auto sm:left-1/2 sm:top-1/2 sm:w-full sm:max-w-md sm:-translate-x-1/2 sm:-translate-y-1/2',
        className,
      )}
      {...props}
    >
...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Title
    ref={ref}
    className={cn('mb-3 text-base font-semibold text-slate-900', className)}
    {...props}
  />
))
